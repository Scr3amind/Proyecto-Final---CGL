...
void CFiguras::prisma (float altura, float largo, float profundidad,  GLuint text, int rep )  //Funcion creacion prisma
{
	...
	GLfloat vertice [8][3] = {
		...
		glBegin(GL_POLYGON);  //Top
			...
			glTexCoord2f(1.0f*rep, 0.0f); glVertex3fv(vertice[5]);
			glTexCoord2f(1.0f*rep, 1.0f*rep); glVertex3fv(vertice[6]);
			glTexCoord2f(0.0f, 1.0f*rep); glVertex3fv(vertice[7]);
		glEnd();
}
